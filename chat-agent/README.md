...
- `KNOWLEDGE_RETRIEVAL_MODE=vector` switches the local demo to deterministic vector-style retrieval with `scoreType="vector"`
- `KNOWLEDGE_LOCAL_FALLBACK=true` lets Spring backend mode fall back to local policy/product knowledge when `/api/knowledge/search` is not available
...
## Optional LLM Answer Refinement

The agent keeps rule-based intent routing and tool selection as the safety layer. An optional LLM can only refine grounded, read-only answers after tools have returned data. Draft actions and mutations are not generated by the LLM.
...
If the key is missing or the provider fails, the agent falls back to deterministic template answers.
